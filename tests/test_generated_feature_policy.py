...
def test_repository_instructions_pin_pipeline_invariants():
    normalized = " ".join(read(INSTRUCTIONS_PATH).split())
    for needle in (
        "request.json",
        "immutable",
        "state_key: candidate",
...
def test_repository_instructions_pin_operator_conventions():
    normalized = " ".join(read(INSTRUCTIONS_PATH).split()).lower()
    for needle in (
        "never hand-edit",
        "flat",
...
def test_repository_instructions_stay_small_and_two_section():
    lines = read(INSTRUCTIONS_PATH).splitlines()
    assert len(lines) <= 60
