...
LOG_DIR="${HOOK_LOG_DIR:-$REPO_ROOT/tmp/hook-logs}"
SESSION_ID=""
TOOL_USE_ID=""
...
# Governed graph artifacts (features/*/graph.yaml, features/*/prompts/*.yaml,
# feature/graph.yaml, feature/prompts/*.yaml) may only be written under an armed
...
def governed_path(path):
    ...
    return bool(
  re.search(r"(^|/)features/[^/]+/graph\.ya?ml$", p)
  or re.search(r"(^|/)features/[^/]+/prompts/[^/]+\.ya?ml$", p)
  or re.search(r"(^|/)feature/graph\.ya?ml$", p)
  or re.search(r"(^|/)feature/prompts/[^/]+\.ya?ml$", p)
    )
...
def terminal_reason(cmd):
    if not re.search(r"features/|feature/", cmd):
        return None
    ...
    for seg in re.split(r"&&|\|\||;|\|", cmd):
        ...
        in_tree = re.match(r"(\./)?(features|feature)(/|$)", dest)
        if not in_tree:
            continue
        for s in sources:
